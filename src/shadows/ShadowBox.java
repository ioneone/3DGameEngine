...
public class ShadowBox {
	...
	protected void update() {
		...
		Vector4f transformedRotation = new Vector4f();
		rotation.transform(FORWARD, transformedRotation);
		Vector3f forwardVector = new Vector3f(transformedRotation.x, transformedRotation.y, transformedRotation.z);
	...
	protected Vector3f getCenter() {
		...
		Matrix4f invertedLight = new Matrix4f().identity();
		lightViewMatrix.invert(invertedLight);
		Vector4f transformedInvertedLight = new Vector4f();
		invertedLight.transform(cen, transformedInvertedLight);
		return new Vector3f(transformedInvertedLight.x, transformedInvertedLight.y, transformedInvertedLight.z);
	}
	...
	private Vector4f[] calculateFrustumVertices(Matrix4f rotation, Vector3f forwardVector,
		...
		Vector4f tempUpVector = new Vector4f();
		rotation.transform(UP, tempUpVector);
		Vector3f upVector = new Vector3f(tempUpVector.x, tempUpVector.y, tempUpVector.z);
	...
	private Vector4f calculateLightSpaceFrustumCorner(Vector3f startPoint, Vector3f direction,
		...
		startPoint.add(
				new Vector3f(direction.x * width, direction.y * width, direction.z * width), point);
