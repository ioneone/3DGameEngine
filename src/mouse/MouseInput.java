...
public class MouseInput {
    ...
    public MouseInput() {
        ...
        scrollValue = new Double(0);
    }

    public void init(Window window) {

        // CursorCallBack only get the position of the mouse through window coordinates rather than pixel coordinates,
        // therefore, multiply the value by windowCoordinatesPixelRate in case the window coordinates and pixel coordinates
        // are not the same.
        glfwSetCursorPosCallback(window.getWindowHandle(), cursorPosCallback = new GLFWCursorPosCallback() {
            @Override
            public void invoke(long window, double xpos, double ypos) {
                currentPos.x = xpos * Window.windowCoordinatesPixelRate;
                currentPos.y = ypos * Window.windowCoordinatesPixelRate;
            }
        });
